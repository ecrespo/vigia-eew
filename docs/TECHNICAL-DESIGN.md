...
### ADR-014 — Country notification filter via offline point-in-polygon (RF-37)

- **Context**: the plain radius filter (RF-12) can't tell countries apart near borders and
  coasts — a quake 80 km away may be in Colombia, Trinidad or a Caribbean island. The user
  asked to not be notified about **other countries'** earthquakes.
- **Decision**: an **opt-in** country filter (`[filter] country_filter`, default `false`).
  Semantics are a **block-list**, not an allow-list: an event is dropped only when it lies
  **positively inside another country**; events over the sea / offshore / of undetermined
  country are **kept** (still subject to radius + magnitude). This is deliberate —
  Venezuela's most dangerous quakes are offshore ("NEAR COAST OF VENEZUELA"), outside any
  land polygon; a strict "must be inside my country" rule would drop exactly those.
- **How the quake's country is found**: `geocode.py::country_of(lat, lon)` does an
  **offline** point-in-polygon lookup against a bundled, reduced **Natural Earth 1:110m**
  dataset (`assets/countries.geojson`, public domain, ~186 KiB, `ISO_A2` + geometry).
  Pure-Python ray casting with a bounding-box pre-check and hole support — **no geospatial
  dependency** (RNF-06 unaffected; the boundary data is a bundled asset like the `.wav`
  files and the tray icon, generated by `packaging/build_countries_geojson.py`). Per-event
  cost is sub-millisecond, well within RNF-01. No per-event network calls.
- **User's country**: derived from the already-resolved reference point
  (`country_of(reference.lat, reference.lon)`) so it works for both manual and IP-detected
  references with no extra network call; overridable with `[filter] country = "VE"`
  (`"auto"` derives it).
- **Fail-safe**: this is a safety tool. If the filter is enabled but the user's country
  can't be determined, `Application._build_geo_filter` leaves the filter **inert** (never
  suppresses). Off by default so existing behavior is unchanged until explicitly enabled.
- **Consequences**: new `geocode.py` and `assets/countries.geojson`; `Filter` gains
  `country_filter`/`country`; `GeoFilter` gains injectable `user_country`/`country_of`
  (inert when absent — backward compatible); `Application` gains `_resolve_user_country`
